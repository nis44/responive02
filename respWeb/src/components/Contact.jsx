...
const Contact = () => {
    ...
    const onSubmit = async (event) => {
      ...
      formData.append("access_key", "2f767b51-aa5e-4c58-b3d0-8298e00139c8");
  ...
  return (
    <motion.div initial = {{opacity:0, x:-200}} transition={{duration:1}} whileInView={{opacity:1, x:0}} viewport={{once:true}} className="text-center p-6 py-20 lg:px-32 w-full overflow-hidden " id="contact">
      ...
      <p className="text-center text-gray-500 mb-12 max-w-90">Ready To Move To Better?</p>

      <form onSubmit={onSubmit} className="max-w-2xl mx-auto text-gray-500 rounded py-3 px-4 mt-2">
        <div className="flex flex-wrap">
            ...
            <div className="w-full md:w-1/2 text-left pl-4">Your Email
        ...
        <div className="my-6 text-left">
            Message
            <textarea name="message" placeholder="Message" required className="w-full border border-gray-500 rounded py-3 px-4 mt-2 h-48 resize-none"></textarea>
        </div>

        <button className="bg-blue-600 py-2 px-12 mb-10 text-white rounded">{result ? result : "Send Message"}</button>
      </form>
    </motion.div>
  )
}
